...
export function defaultStateDir(): string {
  ...
  if (proj && existsSync(proj) && statSync(proj).isDirectory()) {
    return join(proj, ".claude", ".cbt-hooks");
  }
  const user = process.env.USER ?? "anon";
  return join(process.env.TMPDIR ?? tmpdir(), `cbt-hooks-${user}`);
}

export function defaultDbPath(stateDir?: string): string {
  return join(stateDir ?? defaultStateDir(), "cbt.duckdb");
}
...
export function repoRootForState(stateDir: string): string | null {
  ...
  // state_dir is .claude/.cbt-hooks; parent.parent is the repo root.
  const parts = stateDir.split("/");
  if (parts.at(-1) === ".cbt-hooks" && parts.at(-2) === ".claude") {
