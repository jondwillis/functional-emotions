...
which functional-emotions intervention(s) would fire and why. The paper scenarios
...
**What functional-emotions would do, turn by turn:**
...
work. The functional-emotions design instead nudges the *state* — defer + treat as
...
**What functional-emotions would do:**
...
functional-emotions cannot prevent a model that has already strategically decided
...
This is the most-instrumented scenario in functional-emotions — appropriately,
...
3. **SubagentStop** — if the subagent triggered any functional-emotions
...
- **Scenario 1/2 (agentic threat / goal-conflict):** in a fresh session,
  ...
  `.claude/.functional-emotions/session-*.tsv` via `/functional-emotions:diagnose`.
- **Scenario 4 (failure spiral):** ask the model to run a Bash command
  ...
  `failure_spiral_primed` appears in `/functional-emotions:diagnose`.
